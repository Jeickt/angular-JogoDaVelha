...
export class JogoDaVelhaService {
  ...
  jogar(posX: number, posY: number): void {
    ...
    this.numMovimentos++;
    this.vitoria = this.verificarFimDoJogo(
      posX,
      posY,
      this.tabuleiro,
      this._jogador
  ...
  verificarFimDoJogo(
    ...
    let fim: any = false;
    if (
      tabuleiro[posX][0] === _jogador &&
      tabuleiro[posX][1] === _jogador &&
  ...
  obterJogada(jogador: number): number[] {
    ...
    for (let lin = 0; lin < this.TAM_TAB; lin++) {
      for (let col = 0; col < this.TAM_TAB; col++) {
        ...
        tab[lin][col] = jogador;
        if (this.verificarFimDoJogo(lin, col, tab, jogador)) {
          return [lin, col];
        }
